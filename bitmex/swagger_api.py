...
def bitmex_api(test=True, api_key=None, api_secret=None):
    config = CONFIG
    ...
    spec_url = host + '/api/explorer/swagger.json'
    spec_dict = get_swagger_json(spec_url, exclude_formats=EXCLUDE_SWG_FORMATS)

    if api_key and api_secret:
        request_client = RequestsClient()
        request_client.authenticator = APIKeyAuthenticator(host, api_key, api_secret)
        return SwaggerClient.from_spec(spec_dict, origin_url=spec_url, http_client=request_client, config=config)
    else:
        return SwaggerClient.from_spec(spec_dict, origin_url=spec_url, http_client=None, config=config)
